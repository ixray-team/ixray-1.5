...
void CShootingObject::Load	(LPCSTR section)
{
	...
	//Ð²Ñ€ÐµÐ¼Ñ Ð·Ð°Ñ‚Ñ€Ð°Ñ‡Ð¸Ð²Ð°ÐµÐ¼Ð¾Ðµ Ð½Ð° Ð²Ñ‹ÑÑ‚Ñ€ÐµÐ»
...
void CShootingObject::LoadFireParams( LPCSTR section )
{
	...
	//Ð±Ð°Ð·Ð¾Ð²Ð°Ñ Ð´Ð¸ÑÐ¿ÐµÑ€ÑÐ¸Ñ Ð¾Ñ€ÑƒÐ¶Ð¸Ñ
	...
	int num_game_diff_param=_GetItemCount(*s_sHitPower);//ÑƒÐ·Ð½Ð°Ñ‘Ð¼ ÐºÐ¾Ð»Ð»Ð¸Ñ‡ÐµÑÑ‚Ð²Ð¾ Ð¿Ð°Ñ€Ð°Ð¼ÐµÑ‚Ñ€Ð¾Ð² Ð´Ð»Ñ Ñ…Ð¸Ñ‚Ð¾Ð²
	if (num_game_diff_param>1)//ÐµÑÐ»Ð¸ Ð·Ð°Ð´Ð°Ð½ Ð²Ñ‚Ð¾Ñ€Ð¾Ð¹ Ð¿Ð°Ñ€Ð°Ð¼ÐµÑ‚Ñ€ Ñ…Ð¸Ñ‚Ð°
	{
		fvHitPower[egdVeteran]	= (float)atof(_GetItem(*s_sHitPower,1,buffer));//Ñ‚Ð¾ Ð²Ñ‹Ñ‡Ð¸Ñ‚Ñ‹Ð²Ð°ÐµÐ¼ ÐµÐ³Ð¾ Ð´Ð»Ñ ÑƒÑ€Ð¾Ð²Ð½Ñ Ð²ÐµÑ‚ÐµÑ€Ð°Ð½Ð°
	}
	if (num_game_diff_param>2)//ÐµÑÐ»Ð¸ Ð·Ð°Ð´Ð°Ð½ Ñ‚Ñ€ÐµÑ‚Ð¸Ð¹ Ð¿Ð°Ñ€Ð°Ð¼ÐµÑ‚Ñ€ Ñ…Ð¸Ñ‚Ð°
	{
		fvHitPower[egdStalker]	= (float)atof(_GetItem(*s_sHitPower,2,buffer));//Ñ‚Ð¾ Ð²Ñ‹Ñ‡Ð¸Ñ‚Ñ‹Ð²Ð°ÐµÐ¼ ÐµÐ³Ð¾ Ð´Ð»Ñ ÑƒÑ€Ð¾Ð²Ð½Ñ ÑÑ‚Ð°Ð»ÐºÐµÑ€Ð°
	}
	if (num_game_diff_param>3)//ÐµÑÐ»Ð¸ Ð·Ð°Ð´Ð°Ð½ Ñ‡ÐµÑ‚Ð²Ñ‘Ñ€Ñ‚Ñ‹Ð¹ Ð¿Ð°Ñ€Ð°Ð¼ÐµÑ‚Ñ€ Ñ…Ð¸Ñ‚Ð°
	{
		fvHitPower[egdNovice]	= (float)atof(_GetItem(*s_sHitPower,3,buffer));//Ñ‚Ð¾ Ð²Ñ‹Ñ‡Ð¸Ñ‚Ñ‹Ð²Ð°ÐµÐ¼ ÐµÐ³Ð¾ Ð´Ð»Ñ ÑƒÑ€Ð¾Ð²Ð½Ñ Ð½Ð¾Ð²Ð¸Ñ‡ÐºÐ°
	}

	fHitImpulse			= pSettings->r_float	(section, "hit_impulse" );
	//Ð¼Ð°ÐºÑÐ¸Ð¼Ð°Ð»ÑŒÐ½Ð¾Ðµ Ñ€Ð°ÑÑÑ‚Ð¾ÑÐ½Ð¸Ðµ Ð¿Ð¾Ð»ÐµÑ‚Ð° Ð¿ÑƒÐ»Ð¸
	fireDistance		= pSettings->r_float	(section, "fire_distance" );
	//Ð½Ð°Ñ‡Ð°Ð»ÑŒÐ½Ð°Ñ ÑÐºÐ¾Ñ€Ð¾ÑÑ‚ÑŒ Ð¿ÑƒÐ»Ð¸
...
void CShootingObject::LoadFlameParticles (LPCSTR section, LPCSTR prefix)
{
	...
	//Ñ‚ÐµÐºÑƒÑ‰Ð¸Ðµ Ð¿Ð°Ñ€Ñ‚Ð¸ÐºÐ»Ñ‹
...
void CShootingObject::StartFlameParticles	()
{
	...
	//ÐµÑÐ»Ð¸ Ð¿Ð°Ñ€Ñ‚Ð¸ÐºÐ»Ñ‹ Ñ†Ð¸ÐºÐ»Ð¸Ñ‡ÐµÑÐºÐ¸Ðµ
