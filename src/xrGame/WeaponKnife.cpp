...
void CWeaponKnife::KnifeStrike(const Fvector& pos, const Fvector& dir)
{
	...
	shot_targets_t	dest_hits(_malloca(sizeof(Fvector)*m_hits_count), m_hits_count);
...
void CWeaponKnife::MakeShot(Fvector const & pos, Fvector const & dir, float const k_hit)
{
	...
	if (actor->cam_Active() != actor->cameras[eacFirstEye]) {
...
void CWeaponKnife::switch2_Attacking(u32 state)
{
	...
	if(state==eFire2)
		PlayHUDMotion("anm_attack2", FALSE, this, eFire2);
	else //eFire2
		PlayHUDMotion("anm_attack", FALSE, this, eFire);
...
u32 CWeaponKnife::SelectHitsToShot(shot_targets_t & dst_dirs, Fvector const & f_pos)
{
	...
	victims_list_t	tmp_victims_list(
		_malloca(m_spartial_query_res.size() * sizeof(CEntityAlive*)),
	...
	victims_shapes_list_t	tmp_shapes_list(
		_malloca(summ_shapes_count * sizeof(victims_shapes_list_t::value_type)),
		summ_shapes_count);
